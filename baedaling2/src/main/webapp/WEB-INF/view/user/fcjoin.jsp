...
$(function(){
	$(".send_info_btn").click(function(){
		...
		$messageon = $messageline.find("#message_line8");
	    if(!str) {
	    	$messageon.html("<span style='color:red; text-align:centor;'>업체주소를 입력하세요.</span>");
	        f.fcaddr2.focus();
	        return false;
	    }
 ...
 <div id="container_join">
		...
		<form id="" name="fcjoinform" method="post">
			<table>
				...
				<tr class="etch_list2">
					<td class="inputinfo_tr"><input type="text" class="input_normal" id="fczip" name="fczip" style="width: 270px; margin-right: 10px;" readonly="readonly"><button type="button" class="city_search_btn" onclick="daumPostcode2();">주소검색</button></td>
				</tr>
				<tr class="etch_list2">
					<td class="inputinfo_tr"><input type="text" class="input_normal" id="fcaddr1" name="fcaddr1" readonly="readonly"></td>
				</tr>
				<tr class="etch_list2">
...
$(function(){
	$("#find_lck_btn").on('click',function(){
		var addrval = $("#fcaddr1").val().replace(/ *\([^)]*\) */g, "")
        ...
        new daum.Postcode({
            oncomplete: function(data) {
                ...
                if(data.userSelectedType === 'R'){
	                ...
	                // 우편번호와 주소 정보를 해당 필드에 넣는다.
	                document.getElementById('fczip').value = data.zonecode; //5자리 새우편번호 사용
	                document.getElementById('fcaddr1').value = fullAddr;
	                // 커서를 상세주소 필드로 이동한다.
	                document.getElementById('fcaddr2').focus();
